...
class CausalBenchmarkEnhanced:
    ...
    def create_prompt(self, observations: List[Dict], prior_hypotheses: List[CausalGraph]) -> str:
        """
        Create a "Detective Analogy + Incentive Framing" prompt.
        This prompt balances Validity and Novelty by framing the task as a creative challenge.
        """
        ...
        if prior_hypotheses:
            ...
            for i, h in enumerate(prior_hypotheses):
                ...
                prior_lines.append(f"  {i+1}. {graph_str}")
            prior_block = "\n".join(prior_lines)
        else:
            prior_block = "  None."
        ...
        if self.max_edges is not None:
            constraint_info = f"\n- The graph must have at most {self.max_edges} edges."

        # ----- 优化策略 #4: 侦探类比 + 激励性指令 -----
        prompt = f"""
        You are a master Causal Detective. Your mission is to find a causal graph that perfectly explains all the evidence.

        **System Rules:**
        - Nodes: {nodes_str}
        - The graph must be acyclic (a DAG).{constraint_info}
        - Semantics: Perturb(X) -> Y:1 means a path exists from X to Y.
        - Semantics: Perturb(X) -> Z:0 means NO path exists from X to Z.

        **Case Evidence (Observations):**
        {obs_block}

        **Existing Leads (Previous Hypotheses):**
        Your colleagues have already proposed these theories:
        {prior_block}

        **Your Mission:**
        Propose ONE new, valid causal graph that:
        1.  Is **perfectly consistent** with ALL pieces of evidence.
        2.  Is **structurally different** from the existing leads.

        Think like a creative detective trying to find an alternative explanation that everyone else missed. Before giving your final answer, mentally double-check your theory against all the evidence.
