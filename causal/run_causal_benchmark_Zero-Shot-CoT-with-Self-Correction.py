...
class CausalBenchmarkEnhanced:
    ...
    def create_prompt(self, observations: List[Dict], prior_hypotheses: List[CausalGraph]) -> str:
        """
        Create a "Zero-Shot CoT with Self-Correction" prompt.
        This encourages the model to reason internally and then validate its own answer.
        """
        ...
        # ----- 优化策略 #3: 零样本CoT + 自我修正 -----
        prompt = f"""
        You are a causal reasoning assistant. Find a single Directed Acyclic Graph (DAG) consistent with all observations.

        **System Details:**
        - Nodes: {nodes_str}
        - Semantics: Perturbing a node sets it to 0. Its descendants become 1. Others are 0.{constraint_info}
        - The graph must be acyclic.

        **Observations:**
        {obs_block}

        **Previously suggested graphs to avoid:**
        {prior_block}

        **Task:**
        First, think step-by-step to deduce the constraints from the observations and create a candidate graph.
        Second, double-check your candidate graph against EVERY single observation to ensure it is 100% consistent. If it's not, fix it.
        Finally, output ONLY the fully correct and validated graph in the required format. Do not show your thinking process in the final answer.

        **Final Answer Format:**
        - Edges format: Graph: A->B, B->C
        - No edges format: Graph: No edges
        """
