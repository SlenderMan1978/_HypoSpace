# FILE: run_causal_benchmark_optimized.py
# This is a modified version of run_causal_benchmark.py with an optimized prompt.
...
class CausalBenchmarkEnhanced:
    ...
    def create_prompt(self, observations: List[Dict], prior_hypotheses: List[CausalGraph]) -> str:
        ...
        if self.max_edges is not None:
            constraint_info = f"\nConstraint: The graph must have at most {self.max_edges} edges."

        # ----- 优化后的 Prompt (Chain-of-Thought) -----
        prompt = f"""
        You are an expert causal reasoning agent. Your task is to find a Directed Acyclic Graph (DAG) that is consistent with a set of perturbation experiments.

        **System Details:**
        - Nodes in the system: {nodes_str}
        - Semantics: When a node is perturbed, it is set to 0. Any downstream descendant of the perturbed node becomes 1. All other nodes are 0.{constraint_info}

        **Observations:**
        {obs_block}

        **Previously Generated Hypotheses (Avoid these exact structures if possible):**
        {prior_block}

        **Instructions:**
        Follow these steps to find a valid DAG:

        **Step 1: Analyze the observations.** For each observation, deduce the causal relationships.
        For example, if perturbing A causes B to become 1, it implies there is a path from A to B (e.g., A -> ... -> B). If it does NOT cause C to become 1, there is NO path from A to C.

        **Step 2: Synthesize the constraints.** Combine the deductions from all observations into a set of rules that the final graph must obey. For instance, "There must be a path A->B. There cannot be a path B->A. There cannot be a path C->A."

        **Step 3: Construct a candidate DAG.** Based on your analysis from Step 2, construct a single DAG that satisfies ALL constraints. Ensure it is acyclic. Critically, try to propose a graph structure that is DIFFERENT from the previously generated hypotheses.

        **Step 4: Output the final graph.** Provide your answer in the specified format on a single, final line. Do not include your reasoning steps in the final output.

        **Final Answer Format:**
        - If the graph has edges: Graph: A->B, B->C
        - If the graph has no edges: Graph: No edges
        """
        return dedent(prompt).strip()

    # =========================================================================
    # ===== END OF OPTIMIZED PROMPT FUNCTION =====
    # =========================================================================
    ...
    def parse_llm_response(self, response: str) -> Optional[CausalGraph]:
        ...
        line = m.group(1).strip() if m else (s.splitlines()[-1].strip() if s.splitlines() else "")
        if not line:
            return None
    ...
    def run_benchmark(
        ...
        print(f"\nRunning Enhanced Causal Benchmark (Optimized Prompt)")
        ...
        print("\n" + "=" * 60)
        print("ENHANCED BENCHMARK RESULTS SUMMARY (Optimized Prompt)")
        print("=" * 60)
        ...
        if all_errors:
            ...
            if total_error_counts:
                print(f"  Error types:")
                for error_type, count in sorted(total_error_counts.items(), key=lambda x: x, reverse=True):
...
def parse_gt_filter(filter_str: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    if '-' in filter_str and not filter_str.startswith('-'):
        ...
        if len(parts) == 2:
            try:
                min_gt = int(parts.strip())
                max_gt = int(parts.strip())
...
def main():
    parser = argparse.ArgumentParser(
        description="Run enhanced causal discovery benchmark with comprehensive tracking (OPTIMIZED PROMPT VERSION)\n\n"
                    "Features:\n"
    ...
    parser.add_argument("--query-multiplier", type=float, default=1.0,
                        help="Multiplier for adaptive queries. Default matches original run.")
    ...
    if args.output is None:
        ...
        # Add a suffix to distinguish from the original run
        output_pattern = config.get('benchmark', {}).get("output_pattern",
                                                         "results/{dataset_name}_{model}_optimized.json")
    ...
    if args.gt_filter:
        ...
        if gt_filter is not None:
            if gt_filter is not None:
                print(f"Filtering for n_compatible_graphs: [{gt_filter}, {gt_filter}]")
            else:
                print(f"Filtering for n_compatible_graphs: {gt_filter}")
    ...
    # Print configuration
    print("\n" + "=" * 60)
    print("ENHANCED CAUSAL BENCHMARK CONFIGURATION (OPTIMIZED)")
    print("=" * 60)
